...
import players.custom_rhea.utils.customUtilities;

import java.util.*;

import static players.rhea.utils.Constants.*;

public class customEvolution {
    private customRHEAParams params;
    private Random random;

    private customMutation mutationClass;
    private customCrossover crossoverClass;
    private customSelection selectionClass;

    private int nIterations;
    private customIndividual[] population;

    private customGameInterface gInterface;

    public customEvolution(customRHEAParams params, Random random, customGameInterface gInterface) {
        this.params = params;
        this.random = random;
        mutationClass = new customMutation(params, random);
        crossoverClass = new customCrossover(params, random);
        selectionClass = new customSelection(params, random);
        nIterations = 0;
...
//        System.out.println(Arrays.toString(population));
        ...
        // Generate offspring
        customIndividual[] offspring = generate_offspring();
    ...
    private void init_population(int max_actions) {
        population = new customIndividual[params.population_size];
        for (int i = 0; i < params.population_size; i++) {
    ...
    private customIndividual select(customIndividual[] population) {
        return selectionClass.select(population);
    }

    private customIndividual select(customIndividual[] population, customIndividual ignore) {
        customIndividual[] reduced_pop = new customIndividual[population.length - 1];
        int idx = 0;
        for (customIndividual individual : population) {
    ...
    private customIndividual crossover(customIndividual[] population){
        customIndividual parent1 = select(population);
        customIndividual parent2 = select(population, parent1);

        return crossoverClass.cross(parent1, parent2);
    }

    private customIndividual[] generate_offspring() {
        customIndividual[] offspring = new customIndividual[params.offspring_count];
    ...
    private void combine_and_sort_population(customIndividual[] offspring){
        int startIdx = 0;
        ...
        if (params.keep_parents_next_gen) {
            ...
            if (params.reevaluate_pop) {
                for (customIndividual i : population) {
            ...
            offspring = customUtilities.add_array_to_array(population, offspring, startIdx);
