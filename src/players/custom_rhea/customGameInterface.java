...
import players.custom_rhea.evo.customIndividual;
import players.custom_rhea.evo.customMutation;
import players.custom_rhea.hybrids.customMCTSNode;
...
import static players.custom_rhea.utils.customUtilities.*;

public class customGameInterface {

    private StateHeuristic stateHeuristic;
    private customFMBudget fmBudget;
    ...
    customGameInterface(customRHEAParams params, Random random, int playerID) {
        this.params = params;
        this.random = random;
        fmBudget = new customFMBudget(params.fm_budget);
     ...
     */
    public void seed(customIndividual ind, int type) {
        ...
        if (type == INIT_1SLA) {
            ...
            // Random is default
            seed_distribution = customUtilities.getRandomDistribution(params.individual_length, rootState.nActions());
        }
        ind.set_actions(customIndividual.sample_customIndividual(seed_distribution, random).get_actions());
    }
    ...
    private double[][] seed_mcts() {
        ...
        customMCTSNode m_root = new customMCTSNode(random, nActions, actions, stateHeuristic, this);
        customMCTSNode.rootState = rootState;
    ...
    public double evaluate(customIndividual individual, customMutation mutation, int evaluation_update) {
     ...
     */
    private int evaluateRollout(double[] values, GameState copy, int length, customIndividual individual,
    ...
    private double combineAndNormalize(double[] values, double[] values_extension) {
        ...
        if (values_extension != null) {
            if (params.evaluate_act != EVALUATE_ACT_LAST) {
                ...
                values = customUtilities.add_array_to_array(values, values_extension, 0);
            } else {
     ...
     */
    public boolean budget(ElapsedCpuTimer elapsedCpuTimer, int iterationsRemaining, customFMBudget fmBudget) {
    ...
    public void endIteration(ElapsedCpuTimer elapsedCpuTimer, customFMBudget fmBudget) {
        if (params.budget_type == TIME_BUDGET) {
