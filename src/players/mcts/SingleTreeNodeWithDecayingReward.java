...
public class SingleTreeNodeWithDecayingReward
{
    public MCTSParams params;

    private SingleTreeNodeWithDecayingReward parent;
    private SingleTreeNodeWithDecayingReward[] children;
    ...
    SingleTreeNodeWithDecayingReward(MCTSParams p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private SingleTreeNodeWithDecayingReward(MCTSParams p, SingleTreeNodeWithDecayingReward parent, int childIdx, Random rnd, int num_actions,
        ...
        children = new SingleTreeNodeWithDecayingReward[num_actions];
        totValue = 0.0;
    ...
    void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        ...
        while(!stop){
            ...
            SingleTreeNodeWithDecayingReward selected = treePolicy(state);
    ...
    private SingleTreeNodeWithDecayingReward treePolicy(GameState state) {

        SingleTreeNodeWithDecayingReward cur = this;
    ...
    private SingleTreeNodeWithDecayingReward expand(GameState state) {
        ...
        SingleTreeNodeWithDecayingReward tn = new SingleTreeNodeWithDecayingReward(params,this,bestAction,this.m_rnd,num_actions,
    ...
    private SingleTreeNodeWithDecayingReward uct(GameState state) {
        SingleTreeNodeWithDecayingReward selected = null;
        double bestValue = -Double.MAX_VALUE;
        for (SingleTreeNodeWithDecayingReward child : this.children)
        {
            ...
            double uctValue = childValue +
                    ...
                    + rootStateHeuristic.evaluateState(state)/(1 + child.nVisits + params.epsilon);
    ...
    private void backUp(SingleTreeNodeWithDecayingReward node, double result)
    {
        SingleTreeNodeWithDecayingReward n = node;
        while(n != null)
        {
            n.nVisits++;
            n.totValue += result;
    ...
    private boolean notFullyExpanded() {
        for (SingleTreeNodeWithDecayingReward tn : children) {
            if (tn == null) {
                return true;
            }
        }
