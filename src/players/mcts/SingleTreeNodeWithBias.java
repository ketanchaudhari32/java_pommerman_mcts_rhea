...
public class SingleTreeNodeWithBias
{
    public MCTSParams params;

    private SingleTreeNodeWithBias parent;
    private SingleTreeNodeWithBias[] children;
    ...
    SingleTreeNodeWithBias(MCTSParams p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private SingleTreeNodeWithBias(MCTSParams p, SingleTreeNodeWithBias parent, int childIdx, Random rnd, int num_actions,
        ...
        children = new SingleTreeNodeWithBias[num_actions];
        totValue = 0.0;
    ...
    void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        ...
        while(!stop){
            ...
            SingleTreeNodeWithBias selected = treePolicy(state);
    ...
    private SingleTreeNodeWithBias treePolicy(GameState state) {

        SingleTreeNodeWithBias cur = this;
    ...
    private SingleTreeNodeWithBias expand(GameState state) {
        ...
        SingleTreeNodeWithBias tn = new SingleTreeNodeWithBias(params,this,bestAction,this.m_rnd,num_actions,
    ...
    private void backUp(SingleTreeNodeWithBias node, double result)
    {
        SingleTreeNodeWithBias n = node;
        while(n != null)
        {
            n.nVisits++;
            n.totValue += result;
    ...
    private boolean notFullyExpanded() {
        for (SingleTreeNodeWithBias tn : children) {
            if (tn == null) {
                return true;
            }
        }
