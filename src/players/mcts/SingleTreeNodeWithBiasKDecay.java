...
public class SingleTreeNodeWithBiasKDecay
{
    public MCTSParams params;

    private SingleTreeNodeWithBiasKDecay parent;
    private SingleTreeNodeWithBiasKDecay[] children;
    ...
    SingleTreeNodeWithBiasKDecay(MCTSParams p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private SingleTreeNodeWithBiasKDecay(MCTSParams p, SingleTreeNodeWithBiasKDecay parent, int childIdx, Random rnd, int num_actions,
        ...
        children = new SingleTreeNodeWithBiasKDecay[num_actions];
        totValue = 0.0;
    ...
    void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        ...
        while(!stop){
            ...
            SingleTreeNodeWithBiasKDecay selected = treePolicy(state); //selection and expansion
    ...
    private SingleTreeNodeWithBiasKDecay treePolicy(GameState state) {

        SingleTreeNodeWithBiasKDecay cur = this;
    ...
    private SingleTreeNodeWithBiasKDecay expand(GameState state) {
        ...
        SingleTreeNodeWithBiasKDecay tn = new SingleTreeNodeWithBiasKDecay(params,this,bestAction,this.m_rnd,num_actions,
    ...
    private SingleTreeNodeWithBiasKDecay uct(GameState state) {
        SingleTreeNodeWithBiasKDecay selected = null;
        double bestValue = -Double.MAX_VALUE;
        for (SingleTreeNodeWithBiasKDecay child : this.children)
        {
            ...
            double uctValue = childValue +
                    params.K*(highest-lowest) * Math.sqrt(Math.log(this.nVisits + 1) / (child.nVisits + params.epsilon)) // * Math.min(1/4, child.Vsa)
    ...
    private void backUp(SingleTreeNodeWithBiasKDecay node, double result)
    {
        SingleTreeNodeWithBiasKDecay n = node;
        while(n != null)
        {
            n.nVisits++;
            n.totValue += result;
    ...
    private boolean notFullyExpanded() {
        for (SingleTreeNodeWithBiasKDecay tn : children) {
            if (tn == null) {
                return true;
            }
        }
